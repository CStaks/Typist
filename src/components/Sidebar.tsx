  ...
  return (
    <aside className="relative flex h-full shrink-0 border-r border-[var(--border)] bg-[var(--glass)] backdrop-blur-xl">
      <AnimatePresence initial={false}>
        {!collapsed && <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 248, opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ type: "spring", damping: 1.0, stiffness: 180 }} className="overflow-hidden">
          <div className="flex h-full w-[248px] flex-col">
            <div className="flex h-12 items-center justify-between px-4"><span className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">Workspace</span><button aria-label="Collapse sidebar" onClick={() => setCollapsed(true)} className="rounded-lg p-1.5 text-[var(--text-muted)] hover:bg-black/5 dark:hover:bg-white/10"><PanelLeftClose size={16} /></button></div>
            <div className="flex-1 overflow-auto px-2 pb-4">{error ? <p className="px-2 py-3 text-xs text-red-500">{error}</p> : entries.map((entry) => <TreeItem key={entry.path} entry={entry} onSelectFile={onSelectFile} />)}{!isTauri() && <p className="px-2 py-3 text-xs text-[var(--text-muted)]">Open the desktop app to browse a workspace.</p>}</div>
          </div>
        </motion.div>}
      </AnimatePresence>
