  ...
  const { get } = useEditor(
    (root) =>
      Editor.make()
        ...
        .use(gfm)
        .use(listener)
        .config(() => { readyRef.current = true; }),
    [],
  );

  useEffect(() => {
    if (!readyRef.current || value === valueRef.current) return;
    valueRef.current = value;
    get()?.action((ctx) => {
      const view = ctx.get(editorViewCtx);
      const { state } = view;
      const next = state.schema.topNodeType.createAndFill(null, state.schema.text(value)) ?? state.doc;
