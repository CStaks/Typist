...
const cursor = (editor, config) => {
  ...
  const virtualCursor = prosemirrorVirtualCursor.createVirtualCursor({ skipWarning: ["inlineCode"] });
...
const LatexTooltip = vue.defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      return /* @__PURE__ */ vue.h("div", { class: "container" }, props.innerView.value && /* @__PURE__ */ vue.h("div", { ref: innerViewRef }), /* @__PURE__ */ vue.h("button", { type: "button", onPointerdown: onUpdate }, /* @__PURE__ */ vue.h(component.Icon, { icon: props.config.inlineEditConfirm })));
    };
  }
});
...
function resolveKeymapShortcut(ctx, ref) {
  ...
  return formatKeymapShortcut(shortcut, prose.browser.mac);
}
...
function getGroups$1(config, ctx) {
  ...
  groupBuilder.addGroup("formatting", "Formatting").addItem("bold", {
    ...
    keymap: keymapRef(commonmark.strongKeymap.key, "ToggleBold"),
    active: (ctx2) => {
      const commands = ctx2.get(core.commandsCtx);
    ...
    keymap: keymapRef(commonmark.emphasisKeymap.key, "ToggleEmphasis"),
    active: (ctx2) => {
      const commands = ctx2.get(core.commandsCtx);
    ...
    keymap: keymapRef(gfm.strikethroughKeymap.key, "ToggleStrikethrough"),
    active: (ctx2) => {
      const commands = ctx2.get(core.commandsCtx);
      return commands.call(
        commonmark.isMarkSelectedCommand.key,
  ...
  functionGroup.addItem("code", {
    ...
    keymap: keymapRef(commonmark.inlineCodeKeymap.key, "ToggleInlineCode"),
    active: (ctx2) => {
      const commands = ctx2.get(core.commandsCtx);
      return commands.call(
        commonmark.isMarkSelectedCommand.key,
